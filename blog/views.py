...
class UpdateArticleAPIView(APIView):
    def post(self, request, format=None):
        try:
            serialzer = serializers.UpdateArticleCoverSerializer(data=request.data)

            if serialzer.is_valid():
                article_id = serialzer.data.get('article_id')
                cover = request.FILES['cover']
...
class DeleteArticleAPIView(APIView):
    def post(self, request, format=None):
        try:
            serialzer = serializers.DeleteArticleSerelizer(data=request.data)
            if serialzer.is_valid():
                article_id = serialzer.data.get('article_id')
